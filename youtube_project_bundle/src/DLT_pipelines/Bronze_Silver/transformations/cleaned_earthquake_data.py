...
from pyspark.sql.functions import col, count, count_if,from_json,explode,current_timestamp,from_unixtime
import dlt
...
def earthquake_data():
    df=spark.readStream.format('cloudfiles').option('cloudFiles.format','json').load(volume_path).withColumn("_load_timestamp",current_timestamp())
